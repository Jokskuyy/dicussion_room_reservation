...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $date = $_POST['date'];
    $nim = $_POST['nim']; // Pastikan nim diambil dari session yang aktif
    ...
    $nimCheckQuery = "SELECT nim FROM akun WHERE nim='$nim'";
    $nimCheckResult = mysqli_query($koneksi, $nimCheckQuery);



    if ($nimCheckResult && mysqli_num_rows($nimCheckResult) > 0) {
        if ($rowCheckStatus['status'] == 'penuh') {
            echo "<script>alert('Slot sudah penuh, silakan pilih slot lain.');</script>";
            echo "<script>window.location.href='dashboard_dosen.php';</script>";
        } else {
        ...
        echo "<script>alert('NIM tidak terdaftar di upnvj ruang diskusi. Silakan masukkan NIM yang terdaftar.');</script>";
        echo "<script>window.location.href='dashboard_dosen.php';</script>";
    }
}
?>
